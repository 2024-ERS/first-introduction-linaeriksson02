...
m3 <- glm(specrich ~ graztreat, data=dat, family=gaussian(link="identity"))
anova(m3)

# test with generalized linear model assuming now a poisson distribution of residuals 
# if plant species richness is different between the treatments 
m4 <- glm(specrich ~ graztreat, data=dat, family=poisson(link="log"))
anova(m3)
# is this better
anova(m3,m4, test="Chisq")
AIC(m3,m4) # difference not enough
...
m10 <- lme4::glmer(specrich ~ graztreat + (1|block), data=dat, 
                  family=poisson(link="log"))
summary(m10)
anova(m9,m10)
...
m11 <- lme4::glmer(specrich ~ graztreat + (graztreat|block), data=dat, 
                   family=poisson(link="log"))
anova(m10,m11)
...
# -> final conclusion? what should I do? Go lmer with normal distributions is ok
