...
ggplot2::ggplot(data=FactCockleSize, mapping=aes(x=thickness_mm, y=length_mm)) + geom_point(mapping=aes(col=year)) + geom_smooth(method="lm")
```

Make a panel plot where with each year is shown as a separate graph

```{r}
data=FactCockleSize |> dplyr::filter(year %in% c(2014,2015,2017,2018,2020,2021)) |> 
  dplyr::mutate(year=factor(year)) |>
  ggplot2::ggplot(mapping=aes(x=thickness_mm, y=length_mm)) +
